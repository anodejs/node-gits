...
GitSync.prototype.align = function(repoDir, branch, callback) {
    ...
    async.series([
        ...
        // Fetch to update the latest remote state of the branch.
        function(cb) {
            log.log('git fetch origin ' + branch + ':remotes/origin/' + branch + ' ' + repoDir);
            git(repoDir, ['fetch', 'origin', branch + ':remotes/origin/' + branch], function(err, stdout, stderr) {
                if (err) {
