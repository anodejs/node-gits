    ...
    path.exists(dir, function(exists) {
        ...
        if (exists) {
            ...
            self.isValid(dir, function(err, valid) {
                ...
                // if the repository is not valid, remove the directory recursively
                // and try to sync again (which will create it).
                log.warn("A directory with an invalid repository found at " + dir + ". Deleting");
                rimraf(dir, function(err) {
                    if (err) {
                        console.error('Unable to delete invalid repository. This sucks... What to do? oh, what to do?', err);
                        callback({ msg: "Unable to delete invalid repository", err: err });
                        return;
                    }

                    self.sync(url, branch, dir, callback);
                });
            });
